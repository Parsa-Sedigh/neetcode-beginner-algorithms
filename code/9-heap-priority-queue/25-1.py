class Heap:
    ...
    def pop(self):
        ...
        # Note: Keep the original root, since it's gonna get overwritten
        res = self.heap[1]
        ...
        while 2 * i < len(self.heap):
            ...
            # Note: We don't know whether the right child is greater than left or the opposite. So we need to check this.
    ...
    def heapify(self, arr):
        ...
        while cur > 0:
            ...
            # maintain the heap properties for the subtree rooted at `i`:
            while 2 * i < len(self.heap):
                # If the right child exists (2 * i + 1 < len(self.heap)) and is smaller than the
                # left child (self.heap[2 * i + 1] < self.heap[2 * i]), and the current node is larger than the
                # right child (self.heap[i] > self.heap[2 * i + 1]), a swap occurs between the current node and
                # the right child.
                # Note: Why we need to check if right child is less than left child?
                # Because we always want to swap the root with the minimum of left and right children. So if we wanna swap
                # the parent with right child, the right child needs to be smaller than the left(being minimum).
                if (2 * i + 1 < len(self.heap) and
                        self.heap[2 * i + 1] < self.heap[2 * i] and
                        self.heap[i] > self.heap[2 * i + 1]):
                    # Swap right child
            ...
            # We started at the middle of the arr and move backwards
            cur -= 1
